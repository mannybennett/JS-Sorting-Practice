...
const stringsToNumbs = (stringArr) => {
  const numArr = []
  stringArr.forEach(num => numArr.push(parseInt(num)))
  return numArr
}
...
const sumEvens = () => {
  let numArr = stringsToNumbs(strNums)
  return numArr.filter(num => num % 2 === 0).reduce((x, y) => x + y)
}
...
const weatherStates = () => {
  ...
  weather.forEach(obj => uniqueNames.add(obj.weather_state_name))
...
const idealTemp = () => weather.forEach(obj => obj.min_temp === 15.915 ? console.log(obj.id) : false)
